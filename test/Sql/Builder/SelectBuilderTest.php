        ...
        return [
            'Offset0' => [
                ...
                'expected'  => [
                    ...
                    'Oracle' => [
                        'string'     => 'SELECT * FROM (SELECT b.*, rownum b_rownum FROM ( SELECT "foo".* FROM "foo" ) b ) WHERE b_rownum > (0)',
                        'prepare'    => 'SELECT * FROM (SELECT b.*, rownum b_rownum FROM ( SELECT "foo".* FROM "foo" ) b ) WHERE b_rownum > (:offset)',
                        'parameters' => ['offset' => 0],
                    ],
                    'SqlServer' => [
                        'string'     => 'SELECT * FROM ( SELECT [foo].*, ROW_NUMBER() OVER (ORDER BY (SELECT 1)) AS [__ZEND_ROW_NUMBER] FROM [foo] ) AS [ZEND_SQL_SERVER_LIMIT_OFFSET_EMULATION] WHERE [ZEND_SQL_SERVER_LIMIT_OFFSET_EMULATION].[__ZEND_ROW_NUMBER] BETWEEN 0+1 AND 0+0',
                        ...
                        'parameters' => ['offset' => 0, 'limit' => null, 'offsetForSum' => 0],
                    ],
                ],
            ],
            'Limit0' => [
                ...
                'expected'  => [
                    ...
                    'IbmDb2' => [
                        'string'     => 'SELECT * FROM ( SELECT "foo".*, ROW_NUMBER() OVER () AS ZEND_DB_ROWNUM FROM "foo" ) AS ZEND_IBMDB2_SERVER_LIMIT_OFFSET_EMULATION WHERE ZEND_IBMDB2_SERVER_LIMIT_OFFSET_EMULATION.ZEND_DB_ROWNUM BETWEEN 0 AND 0',
                        'prepare'    => 'SELECT * FROM ( SELECT "foo".*, ROW_NUMBER() OVER () AS ZEND_DB_ROWNUM FROM "foo" ) AS ZEND_IBMDB2_SERVER_LIMIT_OFFSET_EMULATION WHERE ZEND_IBMDB2_SERVER_LIMIT_OFFSET_EMULATION.ZEND_DB_ROWNUM BETWEEN ? AND ?',
                        'parameters' => ['offset' => 0, 'limit' => 0],
                    ],
                    'Oracle' => [
                        'string'     => 'SELECT * FROM (SELECT b.*, rownum b_rownum FROM ( SELECT "foo".* FROM "foo" ) b WHERE rownum <= (0+0)) WHERE b_rownum >= (0 + 1)',
                    ...
                    'SqlServer' => [
                        'string'     => 'SELECT * FROM ( SELECT [foo].*, ROW_NUMBER() OVER (ORDER BY (SELECT 1)) AS [__ZEND_ROW_NUMBER] FROM [foo] ) AS [ZEND_SQL_SERVER_LIMIT_OFFSET_EMULATION] WHERE [ZEND_SQL_SERVER_LIMIT_OFFSET_EMULATION].[__ZEND_ROW_NUMBER] BETWEEN 0+1 AND 0+0',
                        'prepare'    => 'SELECT * FROM ( SELECT [foo].*, ROW_NUMBER() OVER (ORDER BY (SELECT 1)) AS [__ZEND_ROW_NUMBER] FROM [foo] ) AS [ZEND_SQL_SERVER_LIMIT_OFFSET_EMULATION] WHERE [ZEND_SQL_SERVER_LIMIT_OFFSET_EMULATION].[__ZEND_ROW_NUMBER] BETWEEN ?+1 AND ?+?',
                        'parameters' => ['offset' => null, 'limit' => 0, 'offsetForSum' => null],
                    ],
                ],
            ],
            'Offset10' => [
            ...
            'Offset10_Limit0' => [
                ...
                'expected'  => [
                    ...
                    'Oracle' => [
                        'string'     => 'SELECT * FROM (SELECT b.*, rownum b_rownum FROM ( SELECT "foo".* FROM "foo" ) b WHERE rownum <= (10+0)) WHERE b_rownum >= (10 + 1)',
                        'prepare'    => 'SELECT * FROM (SELECT b.*, rownum b_rownum FROM ( SELECT "foo".* FROM "foo" ) b WHERE rownum <= (:offset+:limit)) WHERE b_rownum >= (:offset + 1)',
                        'parameters' => ['offset' => 10, 'limit' => 0],
                    ],
                    'SqlServer' => [
                        'string'     => 'SELECT * FROM ( SELECT [foo].*, ROW_NUMBER() OVER (ORDER BY (SELECT 1)) AS [__ZEND_ROW_NUMBER] FROM [foo] ) AS [ZEND_SQL_SERVER_LIMIT_OFFSET_EMULATION] WHERE [ZEND_SQL_SERVER_LIMIT_OFFSET_EMULATION].[__ZEND_ROW_NUMBER] BETWEEN 10+1 AND 0+10',
                        'prepare'    => 'SELECT * FROM ( SELECT [foo].*, ROW_NUMBER() OVER (ORDER BY (SELECT 1)) AS [__ZEND_ROW_NUMBER] FROM [foo] ) AS [ZEND_SQL_SERVER_LIMIT_OFFSET_EMULATION] WHERE [ZEND_SQL_SERVER_LIMIT_OFFSET_EMULATION].[__ZEND_ROW_NUMBER] BETWEEN ?+1 AND ?+?',
            ...
            'Limit10' => [
                ...
                'expected'  => [
                    ...
                    'IbmDb2' => [
                        'string'     => 'SELECT * FROM ( SELECT "foo".*, ROW_NUMBER() OVER () AS ZEND_DB_ROWNUM FROM "foo" ) AS ZEND_IBMDB2_SERVER_LIMIT_OFFSET_EMULATION WHERE ZEND_IBMDB2_SERVER_LIMIT_OFFSET_EMULATION.ZEND_DB_ROWNUM BETWEEN 0 AND 10',
                        'prepare'    => 'SELECT * FROM ( SELECT "foo".*, ROW_NUMBER() OVER () AS ZEND_DB_ROWNUM FROM "foo" ) AS ZEND_IBMDB2_SERVER_LIMIT_OFFSET_EMULATION WHERE ZEND_IBMDB2_SERVER_LIMIT_OFFSET_EMULATION.ZEND_DB_ROWNUM BETWEEN ? AND ?',
            ...
            'Limit10_Offset5' => [
                ...
                'expected'  => [
                    ...
                    'IbmDb2' => [
                        'string'     => 'SELECT * FROM ( SELECT "foo".*, ROW_NUMBER() OVER () AS ZEND_DB_ROWNUM FROM "foo" ) AS ZEND_IBMDB2_SERVER_LIMIT_OFFSET_EMULATION WHERE ZEND_IBMDB2_SERVER_LIMIT_OFFSET_EMULATION.ZEND_DB_ROWNUM BETWEEN 6 AND 15',
                        'prepare'    => 'SELECT * FROM ( SELECT "foo".*, ROW_NUMBER() OVER () AS ZEND_DB_ROWNUM FROM "foo" ) AS ZEND_IBMDB2_SERVER_LIMIT_OFFSET_EMULATION WHERE ZEND_IBMDB2_SERVER_LIMIT_OFFSET_EMULATION.ZEND_DB_ROWNUM BETWEEN ? AND ?',
                        'parameters' => ['offset' => 6, 'limit' => 15],
                    ],
                    'Oracle' => [
